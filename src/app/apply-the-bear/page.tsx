...
function ApplyPageContent() {
  ...
  const castingEmail = 'extras4thebear@gmail.com';
  const castingTitle = 'The Bear Season 5';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ email, password, name, role: 'ACTOR', source: 'the-bear' }),
      });
      ...
      if (res.ok) {
        ...
        localStorage.setItem("landingPageShowName", "The Bear Season 5");
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-sky-900 to-slate-900">
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <div className="inline-block bg-sky-600 text-white px-6 py-2 rounded-full text-sm font-semibold mb-4 animate-pulse">🔴 NOW CASTING • SEASON 5</div>
            <h1 className="text-4xl md:text-6xl font-bold text-white mb-4">FX'S THE BEAR SEASON 5</h1>
            <p className="text-xl md:text-2xl text-gray-300">New roles available • Apply in seconds</p>
          </div>
          <div className="mb-8">
            <div className="aspect-video bg-gray-800 rounded-xl overflow-hidden border-4 border-sky-500 shadow-2xl">
          ...
          <div className="text-center space-y-4 mb-16">
            <Button onClick={scrollToRegister} size="lg" className="w-full md:w-auto text-xl px-12 py-8 bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-700 hover:to-blue-700 text-white font-bold rounded-xl shadow-2xl transform hover:scale-105 transition-all">
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-xl p-8 mb-8">
            ...
            <div className="grid md:grid-cols-3 gap-6">
              ...
              <div className="text-center"><div className="w-12 h-12 bg-sky-500 rounded-full flex items-center justify-center mx-auto mb-3"><Check className="h-6 w-6 text-white" /></div><h3 className="text-white font-semibold mb-2">Never Miss</h3><p className="text-gray-300 text-sm">Get instant notifications for new opportunities</p></div>
            </div>
          </div>
          <div className="text-center mb-8"><p className="text-2xl md:text-3xl font-bold text-white">Join <span className="text-sky-400">1,247 actors</span> already booked through Casting Companion</p></div>
          <div className="mb-8">
          ...
          <div id="register" className="bg-white rounded-xl shadow-2xl p-8 md:p-12 scroll-mt-8">
            <div className="max-w-md mx-auto">
              <div className="text-center mb-8"><h2 className="text-3xl font-bold text-gray-900 mb-2">Complete Your The Bear Application</h2><p className="text-gray-600 text-lg">Create your actor profile and get submitted to The Bear Season 5 PLUS dozens of other matching roles automatically</p></div>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <Button type="submit" disabled={loading} className="w-full h-12 text-lg bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-700 hover:to-blue-700">{loading ? 'Creating Profile...' : 'Create My Profile'}</Button>
                <p className="text-xs text-center text-gray-500">By signing up, you agree to our Terms of Service and Privacy Policy</p>
              </form>
              <div className="mt-6 pt-6 border-t text-center"><p className="text-sm text-gray-600">Already have an account? <Link href="/auth/login" className="text-sky-600 font-semibold hover:underline">Log in</Link></p></div>
            </div>
          </div>
...
export default function ApplyTheBearPage() {
  return (<Suspense fallback={<div className="min-h-screen bg-gradient-to-br from-slate-900 via-sky-900 to-slate-900 flex items-center justify-center"><div className="text-white">Loading...</div></div>}><ApplyPageContent /></Suspense>);
}
